		...
		if(Base.runType.equalsIgnoreCase("-h")){
			...
			for (String[] row : cr.readAll()) {
				...
				for (Map.Entry<String, JsonElement> entry : jsonObject.entrySet()) {
					...
					String errAction = "";
					int exOrder = 0;
					//System.out.println(fieldActions.toString());
					actionList : for(String actionName : fieldActions)
						...
						case "copyData":
							...
							if (fieldLogic.has("copyData")) {
								...
								else if (fieldLogic.getAsJsonObject("copyData").get("action") != null) {
									JsonObject cpdata = fieldLogic.getAsJsonObject("copyData");
									JsonArray cpd_action = cpdata.getAsJsonArray("action");
									for (int i = 0; i < cpd_action.size(); i++) {
										String filter_text = cpd_action.get(i).getAsString();
										if(!filter_text.equalsIgnoreCase("default")) {
											JsonObject value = cpdata.get(filter_text).getAsJsonObject();
											String[] filter = new String[2];
											filter[0] = value.get("filterField").getAsString().trim();
											filter[1] = value.get("filterValue").getAsString().trim();
											String[] target = new String[2];
											target[0] = value.get("targetField").getAsString().trim();
											if(value.get("targetValue") != null)
											target[1] = value.get("targetValue").getAsString().trim();
											copyData.filtermap.put(filter,target);
										} else {
											String filter[] = new String[2];
											filter[0] = "default";
											String target[] = new String[2];
											target[0] = cpdata.get(filter_text).getAsJsonObject().get("targetField").getAsString();
											copyData.filtermap.put(filter,target);
										}
									}
								}
								/*if (fieldLogic.getAsJsonObject("copyData").get("filter") != null) {
									String value = fieldLogic.getAsJsonObject("copyData").get("filter").getAsString();
									copyData.filtermap.put(value.split(":")[0].trim(), value.split(":")[1].trim());
								}*/
							}
				...
				case "curate":
					...
					if (fieldLogic.has("curate")) {
						...
						if (fieldLogic.getAsJsonObject("curate").get("render") != null)
							curate.authorRender = fieldLogic.getAsJsonObject("curate").get("render").getAsBoolean();
						if (fieldLogic.getAsJsonObject("curate").get("removeTokens") != null)
							curate.removeTokens = fieldLogic.getAsJsonObject("curate").get("removeTokens").getAsBoolean();
					}
					fieldActionObjects.add(curate);
					break;
				case "deleteKey":
